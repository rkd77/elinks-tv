		 ...
		 int x, int *y, int w, int *rw, enum format_align align, int format_only)
{
	...
	const char *label = widget_data->widget->text;
	...
	if (label && *label) {
		if (!format_only) text_color = get_bfu_color(term, (const char *)"dialog.text");
	...
	if (label && *label && float_label) {
		if (widget_data->widget->info.field.flags & INPFIELD_FLOAT) {
			(*y) -= INPUTFIELD_HEIGHT;
			dlg_format_text_do(dlg_data, (const char *)INPUTFIELD_FLOAT_SEPARATOR,
		 ...
		 int hide)
{
	...
	color = get_bfu_color(term, (const char *)"dialog.field");
	if (color)
		draw_box(term, &widget_data->box, ' ', 0, color);

	color = get_bfu_color(term, (const char *)"dialog.field-text");
	if (color) {
