...
check_stdio(LIST_OF(struct string_list_item) *url_list)
{
	...
	if (!isatty(STDIN_FILENO)) {
		...
		if (url_list && list_empty(*url_list)) {
			get_opt_bool((const char *)"protocol.file.allow_special_files",
			             NULL) = 1;
			add_to_string_list(url_list, (const char *)"file:///dev/stdin", 17);
		}
		get_cmd_opt_bool((const char *)"no-connect") = 1;
	}

	/* If called for outputting to a pipe without -dump or -source
	 * specified default to using dump viewer. */
	if (!isatty(STDOUT_FILENO)) {
		int *dump = &get_cmd_opt_bool((const char *)"dump");

		if (!*dump && !get_cmd_opt_bool((const char *)"source"))
			*dump = 1;
	}
}
...
init(void)
{
	...
	if (!get_cmd_opt_bool((const char *)"no-home")) {
		init_home();
	}
	...
	if (get_cmd_opt_bool((const char *)"no-connect")
	    || get_cmd_opt_bool((const char *)"dump")
	...
	if (get_cmd_opt_bool((const char *)"dump")
		 ...
		 * context anyway. */
		get_opt_bool((const char *)"ecmascript.enable", NULL) = 0;
#endif
		if (!list_empty(url_list)) {
			dump_next(&url_list);
		} else {
			char *arg = get_cmd_opt_bool((const char *)"dump")
					   ? (char *)"dump" : (char *)"source";
