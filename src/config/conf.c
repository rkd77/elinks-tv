...
skip_white(struct conf_parsing_pos *pos)
{
	...
	while (*start) {
		...
		if (*start == '#') {
			start += strcspn((const char *)start, "\n");
		} else {
			pos->look = start;
			return;
		}
		     ...
		     struct string *mirror, int is_system_conf)
{
	...
	for (handler = parse_handlers; handler->command;
		...
		if (!strncmp((const char *)state->pos.look, (const char *)handler->command, cmdlen)
...
#define FILE_BUF	1024
	...
	fd = open((const char *)name, O_RDONLY | O_NOCTTY);
	if (fd < 0) return NULL;
	set_bin(fd);
                  ...
                  struct terminal *term)
{
	...
	if (ssi) {
		secure_fputs(ssi, (const char *)cfg_str);
