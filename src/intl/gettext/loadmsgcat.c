		     ...
		     struct binding *domainbinding)
{
	...
	if (nullentry != NULL) {
		...
		charsetstr = strstr((char *)nullentry, "charset=");
		if (charsetstr != NULL) {
			size_t len;
...
{
	unsigned char *slash = strrchr((char *)program.path, '/');
...
default:
	...
	if (nullentry != NULL) {
		...
		plural = strstr((char *)nullentry, "plural=");
		nplurals = strstr((char *)nullentry, "nplurals=");
