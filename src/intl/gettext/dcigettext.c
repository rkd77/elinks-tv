...
#define IS_PATH_WITH_DIR(P) (strchr ((char *)P, '/') || strchr ((char *)P, '\\') || HAS_DEVICE (P))
#else
  /* Unix */
#define IS_ABSOLUTE_PATH(P) dir_sep((P)[0])
#define IS_PATH_WITH_DIR(P) strchr ((char *)P, '/')
#endif
	   ...
	   int plural, unsigned long int n, int category)
{
	...
	if (binding == NULL)
		...
		stpcpy(stpcpy(strchr((char *)dirname, '\0'), "/"), binding->dirname);
	}
