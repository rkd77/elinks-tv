...
init_term(int fdin, int fdout)
{
	...
	connectTerminalToWindow(term);
	//term->fWindow = currentFWindow;
...
//			     converted ? converted : title);
	setTerminalTitle(term, converted ? converted : title);
...
extern void setTermItrm(struct terminal *term, struct itrm *itrm);
...
attach_terminal(int in, int out, int ctl, void *info, int len)
{
	...
	itrm = handle_trm(in, out, out, terminal_pipe[1], ctl, info, len, 0);
