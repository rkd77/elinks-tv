...
handle_bittorrent_mode_changes(struct bittorrent_connection *bittorrent)
{
	...
	if (cache->completed_pieces == bittorrent->meta.pieces
		...
		cutoff = get_opt_int((const unsigned char *)"protocol.bittorrent.rarest_first_cutoff",
		                     NULL);
				  ...
				  uint32_t piece)
{
	...
	request_length	= get_opt_int((const unsigned char *)"protocol.bittorrent.peerwire.request_length", NULL);
...
get_bittorrent_file_name(struct bittorrent_meta *meta, struct bittorrent_file *file)
{
	...
	name = expand_tilde(get_opt_str((const unsigned char *)"document.download.directory", NULL));
	if (!name) return NULL;
...
update_bittorrent_piece_cache_state(struct bittorrent_connection *bittorrent)
{
	...
	off_t cache_size = get_opt_int((const unsigned char *)"protocol.bittorrent.piece_cache_size",
	                               NULL);
