...
set_bittorrent_connection_timer(struct connection *conn)
{
	...
	milliseconds_T interval = sec_to_ms(get_opt_int((const unsigned char *)"protocol.bittorrent.choke_interval", NULL));
...
update_bittorrent_connection_state(struct connection *conn)
{
	...
	int min_uploads = get_opt_int((const unsigned char *)"protocol.bittorrent.min_uploads", NULL);
	int max_uploads = get_opt_int((const unsigned char *)"protocol.bittorrent.max_uploads", NULL);
	...
	max_peer_conns = get_opt_int((const unsigned char *)"protocol.bittorrent.peerwire.connections", NULL);
	...
	if (!list_empty(bittorrent->peers)) {
		struct bittorrent_peer *peer_info, *next_peer_info;
		int pool_size = get_opt_int((const unsigned char *)"protocol.bittorrent.peerwire.pool_size", NULL);
		int pool_peers = 0;
